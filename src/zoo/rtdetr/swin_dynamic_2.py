...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Store original shortcut before any padding
        shortcut = x

        # Handle arbitrary input sizes by padding if necessary
        pad_l = pad_t = 0
        ...
        # Remove padding if added to match original input size
        if pad_r > 0 or pad_b > 0:
            x = x[:, :H, :W, :].contiguous()

        # FFN - now both shortcut and x have the same dimensions
...
if __name__ == "__main__":
    ...
    x1 = torch.randn(2, 56, 56, 96)  # B=2, H=56, W=56, C=96
    x2 = torch.randn(2, 28, 28, 96)  # B=2, H=28, W=28, C=96
    x3 = torch.randn(2, 64, 32, 96)  # B=2, H=64, W=32, C=96 (non-square)
    ...
    print(f"Input 3 shape: {x3.shape}")
    out3 = swin_block(x3)
